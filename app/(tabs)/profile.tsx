...
export default function ProfileScreen() {
  ...
  const handleShare = () => {
    Alert.alert('Paylaş', 'Profil bağlantısı kopyalandı!');
  };
             ...
             <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.activitiesScroll}>
                 {profile.history.map((item: any) => (
                    <ActivityCard 
                        key={item.id}
                        isDark={isDark}
                        image="https://images.unsplash.com/photo-1579952363873-27f3bade9f55?ixlib=rb-1.2.1&auto=format&fit=crop&w=500&q=60"
                        date={item.date}
                        icon={item.icon}

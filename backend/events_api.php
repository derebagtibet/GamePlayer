...
if ($method == 'GET') {
    if ($endpoint == 'dashboard') {
        ...
        try {
            ...
            // Oyuncu arayan etkinlikler (Gerçek veri)
            ...
            // Aktiviteler (Bildirimlerden çekiliyor)
            ...
            foreach ($db_activities as $act) {
                // Zamanı formatla (Örnek: "2 dk önce") - Basitçe saat/tarih gönderelim, frontend formatlasın veya burada basit bir format yapalım
                ...
                $item = [
                    "id" => $act['id'],
                    "type" => $act['type'] == 'invite' ? 'invite' : 'system', // Frontend bu tipleri bekliyor
                    "text" => $act['text'],
                    "time" => $time_str
                ];

                if ($act['type'] == 'invite') {
                    // Davet eden kişinin adını metinden veya ilişkiden bulmak lazım. Şimdilik metinden varsayalım veya sabit bir avatar verelim.
                    // Veritabanı yapısında related_id var ama kimin davet ettiği net değil, title "Maç Daveti" olabilir.
                    $item['avatar'] = "Admin"; // Varsayılan
                } else {
                    $item['icon'] = "notifications"; // Varsayılan ikon
                    if ($act['type'] == 'system') $item['icon'] = "info";
                    if ($act['type'] == 'alert') $item['icon'] = "warning";
                }
                $activities[] = $item;
            }

            // 4. Yaklaşan Etkinlikler (Genel)
        ...
        // Gerçek katılımcı sayıları ile maçlarım
        $sql_u = "
        ...
        try {
            ...
            foreach ($upcoming as &$m) {
                ...
                // Küçük resimler için isim simülasyonu
                $m['avatars'] = ['User'];
    ...
    // Eğer user_id body içinde geldiyse ve GET'te yoksa onu kullan
    ...
    if ($endpoint == 'join') {
        if (isset($data->event_id) && $current_user_id > 0) {
            try {
                ...
                try {
                    $pdo->exec("ALTER TABLE event_participants ADD COLUMN position VARCHAR(50) DEFAULT NULL");
                } catch (Exception $e) {}
                ...
                $pdo->prepare($sql)->execute([$data->event_id, $current_user_id, $position]);
        ...
        if (isset($data->event_id) && $current_user_id > 0) {
            try {
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    echo json_encode(["status" => "error", "message" => "Katılım bulunamadı veya zaten ayrıldınız."]);
                }
            } catch (PDOException $e) { echo json_encode(["status" => "error", "message" => $e->getMessage()]); }
        }
        ...
        if(isset($data->title) && isset($data->date)) {
            ...
            try {
                $pdo->prepare($sql)->execute([$current_user_id, $data->title, $data->category ?? 'Futbol', $data->date, $data->location, $data->description ?? '', $data->price ?? 0, $data->type ?? 'friendly']);
                $event_id = $pdo->lastInsertId();
                // Organizatörü otomatik katıldı yap
